...
if (isset($_GET['action'])){//Actualizacion de los datos
	...
	if ($campo==1){
		...
		$valor=intval($_REQUEST['valor']);
		$sentencia_sql="id_plantilla='$valor'";
	} else if ($campo==10){
		echo $valor=floatval($_REQUEST['valor']);
		$sentencia_sql="impuestos='$valor'";
	} else {
		$sentencia_sql="";
	}
...
$sql_taxes=mysqli_query($con,"select * from impuestos where status=1");
...
<?php
	...
	if (isset($id_detalle_cotizacion) and $id_detalle_cotizacion==$id_tmp)
	{
		...
		<tr class="<?php echo $clase;?>">
			...
			<td class='text-right'><?php echo number_format($precio_unitario,$decimals, $dec_point, $thousands_sep);?></td>
			<td class='text-right'><?php echo number_format($total_descuento,$decimals, $dec_point, $thousands_sep);?></td>
			<td class='text-right'><?php echo number_format($precio_total,$decimals,$dec_point,$thousands_sep);?></td>
			<td class='text-right'>
				...
				<a href="#" onclick="eliminar('<?php echo $id_tmp ?>')"><i class="fa fa-trash"></i></a>
			</td>
		</tr>		
		<?php
		$validez=$row['validez'];
...
?>
<tr>
	<td colspan=5 class='text-right'>PARCIAL <?php echo $moneda;?></td>
	<td class='text-right'><?php echo number_format($total_parcial,$decimals,$dec_point,$thousands_sep);?></td>
	<td></td>
</tr>
<tr>
	<td colspan=5 class='text-right'>DESCUENTO <?php echo $moneda;?></td>
	<td class='text-right'><?php echo number_format($sumador_descuento,$decimals,$dec_point,$thousands_sep);?></td>
	<td></td>
</tr>
<tr>
	<td colspan=5 class='text-right'>NETO <?php echo $moneda;?></td>
	<td class='text-right'><?php echo number_format($total_neto,$decimals,$dec_point,$thousands_sep);?></td>
	<td></td>
</tr>
<tr>
	<td colspan=5 class='text-right'>
		<select name="taxes" id="taxes" onchange="update_cotizacion(10,this.value);" class='form-control form-control-sm' style="max-width:120px">
		<?php
	...
	<td class='text-right'><?php echo number_format($total_iva,$decimals,$dec_point,$thousands_sep);?></td>
	<td></td>
</tr>
<tr>
	<td colspan=5 class='text-right'>TOTAL <?php echo $moneda;?></td>
	<td class='text-right'><?php echo number_format($total_cotizacion,$decimals,$dec_point,$thousands_sep);?></td>
	<td></td>
</tr>
</table>
